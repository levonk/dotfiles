...
[[ -n "${DEBUG_MODULE_LOADING:-}" ]] && print -u2 -- "[DEBUG] Evaluating P10K theme path: ${P10K_OMZ_THEME_FILE}"
if [[ -r "${P10K_OMZ_THEME_FILE}" ]]; then
  __set_theme "powerlevel10k/powerlevel10k"
  export POWERLEVEL9K_DISABLE_CONFIGURATION_WIZARD=true
  [[ -n "${DEBUG_MODULE_LOADING:-}" ]] && print -u2 -- "[DEBUG] Selected theme: ${ZSH_THEME} (via custom path)"
else
  __set_theme "robbyrussell"
  [[ -n "${DEBUG_PROMPT}" ]] && echo "[omz] P10K theme not found under $ZSH_CUSTOM/themes; using fallback: ${ZSH_THEME}" >&2
  [[ -n "${DEBUG_MODULE_LOADING:-}" ]] && print -u2 -- "[DEBUG] P10K theme not found; fallback theme: ${ZSH_THEME}"
fi
...
if [[ -r "$ZSH/oh-my-zsh.sh" ]]; then
  ...
  [[ -n "${DEBUG_MODULE_LOADING:-}" ]] && print -u2 -- "[DEBUG] Attempting to load module: oh-my-zsh ($ZSH/oh-my-zsh.sh)"
  source "$ZSH/oh-my-zsh.sh"
  [[ -n "${DEBUG_MODULE_LOADING:-}" ]] && print -u2 -- "[DEBUG] Successfully loaded module: oh-my-zsh"
else
  [[ -n "${DEBUG_MODULE_LOADING:-}" ]] && print -u2 -- "[DEBUG] Skipping module (not readable): oh-my-zsh ($ZSH/oh-my-zsh.sh)"
fi
...
if [[ "${ZSH_THEME:-}" == "powerlevel10k/powerlevel10k" ]]; then
  if ! typeset -f prompt_powerlevel10k_setup >/dev/null 2>&1; then
    ...
    if [[ -r "$P10K_THEME_FILE" ]]; then
      ...
      [[ -n "${DEBUG_MODULE_LOADING:-}" ]] && print -u2 -- "[DEBUG] Attempting to load module: p10k theme (plugins) ($P10K_THEME_FILE)"
      source "$P10K_THEME_FILE"
      [[ -n "${DEBUG_MODULE_LOADING:-}" ]] && print -u2 -- "[DEBUG] Successfully loaded module: p10k theme (plugins)"
    elif [[ -r "$P10K_THEME_FILE_CUSTOM" ]]; then
      [[ -n "${DEBUG_PROMPT}" ]] && echo "[omz] sourcing P10K theme directly (custom): $P10K_THEME_FILE_CUSTOM" >&2
      # shellcheck disable=SC1091
      [[ -n "${DEBUG_MODULE_LOADING:-}" ]] && print -u2 -- "[DEBUG] Attempting to load module: p10k theme (custom) ($P10K_THEME_FILE_CUSTOM)"
      source "$P10K_THEME_FILE_CUSTOM"
      [[ -n "${DEBUG_MODULE_LOADING:-}" ]] && print -u2 -- "[DEBUG] Successfully loaded module: p10k theme (custom)"
    else
