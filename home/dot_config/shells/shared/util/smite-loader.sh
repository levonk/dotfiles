...
case "${ZSH_VERSION:+zsh}${BASH_VERSION:+bash}" in
  zsh)
    ...
    module_path="$SHELLS_BASE_DIR/zsh/util/smite.zsh"
    ;;
  bash)
    module_name="bash_util_smite"
    module_path="$SHELLS_BASE_DIR/bash/util/smite.bash"
    ;;
  *)
    # Try to infer from $SHELL
    case "${SHELL##*/}" in
      zsh)
        module_name="zsh_util_smite"; module_path="$SHELLS_BASE_DIR/zsh/util/smite.zsh" ;;
      bash)
        module_name="bash_util_smite"; module_path="$SHELLS_BASE_DIR/bash/util/smite.bash" ;;
      *)
...
if [ -r "$module_path" ] && ! command -v smite >/dev/null 2>&1; then
  register_lazy_module "$module_name" "$module_path" "smite"
fi
