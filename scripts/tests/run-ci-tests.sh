...
run_chezmoi_test_for_user() {
    ...
    sudo -H -u "$user" /bin/bash -c 'set -euo pipefail; export PATH=/usr/local/bin:/usr/bin:/bin; export CHEZMOI_NO_SHELL_SWITCH=1; git config --global --add safe.directory /workspace; timeout 60s /usr/local/bin/chezmoi init --apply --source /workspace --verbose --debug' > "$chezmoi_log" 2>&1
